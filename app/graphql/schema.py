...
from strawberry.types import Info  # <--- 1. Add this import
...
class Query:
    ...
    async def patients(self, info: Info) -> List[PatientType]:
        ...
        rows = (
            (await db.execute(select(Patient).order_by(Patient.id.desc())))
            .scalars()
            .all()
        )
        return [
            PatientType(id=p.id, patient_no=p.patient_no, full_name=p.full_name)
            for p in rows
        ]
    ...
    async def tests(self, info: Info) -> List[TestType]:
        ...
        return [
            TestType(
                id=t.id,
                name=t.name,
                price_ghs=float(t.price_ghs) if t.price_ghs is not None else None,
            )
            for t in rows
        ]
    ...
    async def analyzers(self, info: Info) -> List[AnalyzerType]:
        db: AsyncSession = info.context["db"]
        rows = (
            (await db.execute(select(Analyzer).order_by(Analyzer.name))).scalars().all()
        )
