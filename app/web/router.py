...
def _render(request: Request, template_name: str, *, active_page: str | None = None, active_group: str | None = None, **extra):
    ctx = {
        "request": request,
...
async def patients_page(
    ...
    tpl = "all-patients-list.html" if (settings.TEMPLATES_PATH / "all-patients-list.html").exists() else "all-patients.html"
    return _render(request, tpl, active_page="patients", patients=patients, q=q or "")
...
async def patient_add_get(request: Request, db: AsyncSession = Depends(get_db)):
    ins = (await db.execute(select(InsuranceCompany).order_by(InsuranceCompany.name.asc()))).scalars().all()
    return _render(request, "add-patient.html", active_page="patients", insurance_companies=ins)
...
@router.get("/patients/{patient_id}", response_class=HTMLResponse, name="patient_detail")
async def patient_detail(request: Request, patient_id: int, db: AsyncSession = Depends(get_db)):
    ...
    tpl = "patient-details.html" if (settings.TEMPLATES_PATH / "patient-details.html").exists() else "all-patients.html"
...
async def patient_new_visit_get(request: Request, patient_id: int, db: AsyncSession = Depends(get_db)):
    patient = (await db.execute(select(Patient).where(Patient.id == patient_id))).scalar_one_or_none()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
...
async def patient_new_visit_post(request: Request, patient_id: int, reason: str | None = Form(None), db: AsyncSession = Depends(get_db)):
    patient = (await db.execute(select(Patient).where(Patient.id == patient_id))).scalar_one_or_none()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
...
@router.get("/patients/{patient_id}/appointments/add", response_class=HTMLResponse, name="appointment_add")
async def appointment_add_get(request: Request, patient_id: int, db: AsyncSession = Depends(get_db)):
    patient = (await db.execute(select(Patient).where(Patient.id == patient_id))).scalar_one_or_none()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    return _render(request, "appointment-add.html", active_page="appointments", patient=patient)
...
@router.get("/patients/{patient_id}/book-lab", response_class=HTMLResponse, name="patient_book_lab")
async def patient_book_lab_get(request: Request, patient_id: int, db: AsyncSession = Depends(get_db)):
    patient = (await db.execute(select(Patient).where(Patient.id == patient_id))).scalar_one_or_none()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    ...
    return _render(request, "book-lab.html", active_page="lab_main", active_group="lab", patient=patient, tests=tests)
...
async def patient_book_lab_post(
    ...
    # Load tests
    tests = (await db.execute(select(Test).where(Test.id.in_(test_ids)))).scalars().all()
    if not tests:
    ...
    for t in tests:
        price = float(t.price_ghs or 0)
        db.add(InvoiceItem(invoice_id=invoice.id, test_id=t.id, description=t.name, unit_price=price, qty=1, line_total=price))

    await db.commit()
    return RedirectResponse(url=request.url_for("invoice_detail", invoice_id=invoice.id), status_code=303)


@router.get("/invoice/{invoice_id}", response_class=HTMLResponse, name="invoice_detail")
async def invoice_detail(request: Request, invoice_id: int, db: AsyncSession = Depends(get_db)):
...
async def invoice_pay(
    ...
    # add payment
    pay = Payment(invoice_id=invoice.id, amount=amount, method=method, reference=reference)
    db.add(pay)
    await db.flush()
    ...
    if invoice.status == "paid" and invoice.order_id:
        items = (await db.execute(select(LabOrderItem).where(LabOrderItem.order_id == invoice.order_id))).scalars().all()
        for it in items:
            if it.stage == "booking":
                db.add(LabStatusLog(order_item_id=it.id, from_stage=it.stage, to_stage="sampling", note="Auto: payment confirmed"))
                it.stage = "sampling"
...
async def lab_main(request: Request):
    return _render(request, "laboratory.html", active_page="lab_main", active_group="lab")

@router.get("/lab/results", response_class=HTMLResponse, name="lab_results")
async def lab_results(request: Request):
    tpl = "lab-results.html" if (settings.TEMPLATES_PATH / "lab-results.html").exists() else "laboratory.html"
...
async def reports(request: Request):
    tpl = "reports.html" if (settings.TEMPLATES_PATH / "reports.html").exists() else "transaction-report.html"
...
async def staff_list(request: Request):
    tpl = "staffs.html" if (settings.TEMPLATES_PATH / "staffs.html").exists() else "all-doctors-list.html"
...
async def invoice_list(request: Request):
    tpl = "invoice.html" if (settings.TEMPLATES_PATH / "invoice.html").exists() else "add-invoice.html"
    return _render(request, tpl, active_page="invoice")

@router.get("/payments", response_class=HTMLResponse, name="payments")
async def payments(request: Request):
    tpl = "transactions.html" if (settings.TEMPLATES_PATH / "transactions.html").exists() else "add-payment.html"
...
async def notifications(request: Request):
    tpl = "notifications.html" if (settings.TEMPLATES_PATH / "notifications.html").exists() else "notifications-settings.html"
    return _render(request, tpl, active_page="notifications")

@router.get("/messages", response_class=HTMLResponse, name="messages")
async def messages(request: Request):
    tpl = "messages.html" if (settings.TEMPLATES_PATH / "messages.html").exists() else "chat.html"
...
@router.get("/settings/test-mapping", response_class=HTMLResponse, name="settings_test_mapping")
async def settings_test_mapping(request: Request, db: AsyncSession = Depends(get_db)):
    analyzers = (await db.execute(select(Analyzer).where(Analyzer.is_active == True).order_by(Analyzer.name.asc()))).scalars().all()
    tests = (await db.execute(
        select(Test)
        .options(selectinload(Test.default_analyzer))
...
async def settings_test_mapping_post(
    ...
    await db.commit()
    return RedirectResponse(url=request.url_for("settings_test_mapping"), status_code=303)


# --- Settings: Analyzer registration (dynamic connectivity) ---

@router.get("/settings/analyzers", response_class=HTMLResponse, name="settings_analyzers")
async def settings_analyzers(request: Request, db: AsyncSession = Depends(get_db)):
    analyzers = (await db.execute(select(Analyzer).order_by(Analyzer.name.asc()))).scalars().all()
    return _render(
        request,
...
async def analyzer_add_post(
    ...
    exists = (await db.execute(select(Analyzer).where(Analyzer.name == name.strip()))).scalar_one_or_none()
    if exists:
...
@router.get("/settings/analyzers/{analyzer_id}/edit", response_class=HTMLResponse, name="analyzer_edit")
async def analyzer_edit_get(request: Request, analyzer_id: int, db: AsyncSession = Depends(get_db)):
    analyzer = (await db.execute(select(Analyzer).where(Analyzer.id == analyzer_id))).scalar_one_or_none()
    if not analyzer:
...
async def analyzer_delete(request: Request, analyzer_id: int, db: AsyncSession = Depends(get_db)):
    analyzer = (await db.execute(select(Analyzer).where(Analyzer.id == analyzer_id))).scalar_one_or_none()
    if analyzer:
...
async def tasks_board(
    ...
    if q and q.strip():
        like = f"%{q.strip()}%"
        stmt = stmt.join(LabOrder, LabOrderItem.order_id == LabOrder.id).join(Patient, LabOrder.patient_id == Patient.id)
    ...
    invoices = {}
    if order_ids:
        invs = (await db.execute(select(Invoice).where(Invoice.order_id.in_(order_ids)))).scalars().all()
        invoices = {inv.order_id: inv for inv in invs if inv.order_id}

    # Group items by stage
    grouped: dict[str, list] = {k: [] for k, _ in STAGE_ORDER}
    for it in items:
        grouped.setdefault((it.stage.value if hasattr(it.stage, 'value') else (it.stage or 'booking')), []).append(it)
...
async def task_update_stage_api(
    ...
    it = (await db.execute(select(LabOrderItem).where(LabOrderItem.id == item_id))).scalar_one_or_none()
    if not it:
    ...
    from_stage = (it.stage.value if hasattr(it.stage, "value") else (it.stage or "booking"))
    if to_stage == from_stage:
        return {"ok": True, "stage": from_stage}

    allowed = ALLOWED_TRANSITIONS.get(from_stage, set())
    if to_stage not in allowed:
        raise HTTPException(status_code=400, detail=f"Not allowed: {from_stage} -> {to_stage}")

    # Payment gate: Booking -> Sampling requires paid invoice
    inv = None
    if from_stage == "booking" and to_stage == "sampling":
        inv = (await db.execute(select(Invoice).where(Invoice.order_id == it.order_id))).scalar_one_or_none()
        if not inv or inv.status != "paid":
            raise HTTPException(status_code=400, detail="Invoice must be PAID before sampling")
    ...
    # Log status change
    db.add(LabStatusLog(order_item_id=it.id, from_stage=from_stage, to_stage=to_stage, note=note))
    await db.commit()

    # Notify patient via email (if available)
    try:
        order = (await db.execute(
            select(LabOrder).where(LabOrder.id == it.order_id).options(selectinload(LabOrder.patient))
        )).scalar_one_or_none()
...
async def task_update_stage(
    ...
    allowed = ALLOWED_TRANSITIONS.get(from_stage, set())
    if to_stage not in allowed:
        raise HTTPException(status_code=400, detail=f"Stage transition not allowed: {from_stage} -> {to_stage}")
    ...
    if from_stage == "booking" and (not inv or inv.status != "paid"):
        raise HTTPException(status_code=400, detail="Payment is required before proceeding to the next step")
