...
function render(queueList) {
  ...
  if (!window.jQuery || !$.fn.DataTable) {
      console.warn("DataTable plugin not ready yet. Retrying in 100ms...");
  ...
  if ($.fn.DataTable.isDataTable(tableSelector)) {
    ...
    if (!queueList || queueList.length === 0) {
        phlebotomyContainerEl.innerHTML = `<tr><td colspan="7" class="text-center py-5 text-muted">No pending collections for this date range.</td></tr>`;
        return;
    }
