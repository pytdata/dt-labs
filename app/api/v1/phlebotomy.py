...
from app.models.catalog import Phlebotomy, Sample, SampleTest
...
async def create_sample(data: SampleCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Phlebotomy).where(Phlebotomy.id == data.phlebotomy_id)
    )
    phleb = result.scalar_one_or_none()

    if not phleb:
        raise HTTPException(status_code=404, detail="Phlebotomy record not found")

    sample = Sample(
        phlebotomy_id=phleb.id,
        sample_type=data.sample_type,
        appointment_id=phleb.appointment_id,
        patient_id=phleb.patient_id,
    )
    db.add(sample)
    await db.flush()

    for test_id in data.test_ids:
        link = SampleTest(sample_id=sample.id, test_id=test_id)
        db.add(link)
